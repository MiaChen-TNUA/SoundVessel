# Visualized Circle 展示網頁

一個為 `visualizedCirle02` 影片量身打造的高質感展示網頁，採用極簡暗黑美學與響應式設計，非常適合直接託管在 GitHub Pages 上供他人瀏覽。
...
- **全視窗自適應 (Responsive Layout)**：使用 CSS 視窗比例單位 `vmin` 設計。播放器會自動感應螢幕的寬度與高度，並自動調整為最合適的尺寸展示，在大螢幕或手機直式螢幕上皆能置中完美呈現，無須手動捲動頁面。
- **背景自動循環播放**：啟用 `autoplay`、`muted`、`loop` 與 `playsinline` 屬性，讓影片在進入網頁時即可自動開始無縫循環播放。
- **極簡點擊靜音切換**：沒有複雜的控制項，使用者只需點擊**影片本身**或右下角的**喇叭懸浮按鈕**，即可在「靜音 / 有聲」之間自由切換。
